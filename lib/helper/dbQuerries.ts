...
export async function fetchDataModificationL3(tarifdb: string, hwdb: string): Promise<{[key: string]: string}> {
    let connection;
    ...
    try {
        ...
        WHERE 1=1
        AND tariff.PRODUCT_CODE = :tarifdb
        ...
        const result = await connection.execute(query, { tarifdb, hwdb });
