...
import { createModificationChangeTariffL1OrderBody, createModificationSwapHWL1OrderBody, createModificationSwapHWL3OrderBody } from "../../lib/datafactory/createOrder"
import { serviceOrderL3Provisioning, serviceOrderClosed, TEMPserviceOrderL1Provisioning, serviceOrderProvisioning } from "../../lib/datafactory/serviceOrder";
...
test.describe("Modifikace L1 - Změna tarifu",async () => {
    L1config.allowedCombinationForChange.forEach(config => {
        test(`Změna z tarifu ${config.originalTariff} na tarif ${config.newTariff} s HW typu ${config.originalHW}`, async ({ request }) => {
            ...
            await test.step("Create", async () => {
                let requestBody = await createModificationChangeTariffL1OrderBody(idASSET_sub, idASSET_ser, WHSDATA, idASSET_ass1, WHSHW_OG, idASSET_ass2, macAddress, config.newTariff);
