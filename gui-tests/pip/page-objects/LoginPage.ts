...
class LoginPage {
    ...
    async enterUsername(username: string) {
        const inputElement = await this.getElementByXPath('//div[@id="weColorPanel8"]//input[contains(@class, "weEditDark") and @type="text"][1]');
        await inputElement.fill(username);
    }

    async enterPassword(password: string) {
        const inputElement = await this.getElementByXPath('//div[@id="weColorPanel8"]//input[contains(@class, "weEditDark") and @type="password"][1]');
