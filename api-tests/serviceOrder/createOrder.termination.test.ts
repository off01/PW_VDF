...
import { createTerminationL3OrderBody } from "../../lib/datafactory/createOrder"
...
import { fetchOrderIdTermination } from "../../lib/helper/dbQuerries";


test.describe("Aktivace test",async () => {
    test('Terminační objenávka pro WHSDATA009 L3', async ({ request }) => {
        const idASSET_sub = await fetchOrderIdTermination();
        if (!idASSET_sub) {
